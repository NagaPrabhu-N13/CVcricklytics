...
function Tournamentseries() {
  ...
  const handleNavigation = async (e) => {
    ...
    const isFormValid = (
      ...
      noOfTeams.trim() !== '' && 
      startDate !== null &&  // Changed to check for null instead of ''
      endDate !== null &&    // Changed to check for null instead of ''
    ...
    // Convert dates to a simple string format (YYYY-MM-DD) for storage
    const formattedStartDate = startDate ? startDate.toISOString().split('T')[0] : null;
    const formattedEndDate = endDate ? endDate.toISOString().split('T')[0] : null;

    // Prepare the tournament data with userId and formatted dates
    const tournamentData = {
      name: tournamentName,
      location: selectedLocation,
      noOfTeams: noOfTeams,
      startDate: formattedStartDate,  // Stored as simple string
      endDate: formattedEndDate,      // Stored as simple string
